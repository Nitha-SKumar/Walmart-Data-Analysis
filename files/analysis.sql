...
-- each product line and add a column to those product line showing "Good", "Bad". Good if its greater than average sales
WITH avg_sales AS (
...
--  unique customer types does the data have
SELECT 
	DISTINCT(customer_type) AS customer 
FROM sales;
-- unique payment methods does the data have
SELECT 
...
WITH AverageRatingsPerDay AS (
    SELECT 
        branch,
        day_name,  -- Use the existing day_name column
        AVG(rating) AS avg_rating
    FROM sales
...
-- Estimate the total value a customer brings over their entire relationship with the business.
...
-- Segment customers into di


-- stinct groups based on purchasing behavior and demographics using RFM analysis.
...
-- Analyze customer feedback and ratings to gauge sentiment.
...
-- Analyze revenue contribution by different payment methods.
...
-- Analyze customer purchase patterns based on gender and customer type.
